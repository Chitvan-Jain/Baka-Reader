...
export default function SearchPage() {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 md:px-6 py-6 animate-fade-in">
      <h1 className="text-2xl font-bold text-text-primary mb-6">Search Manga</h1>
      ...
      <form onSubmit={handleSearch} className="flex gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-11 pl-10 pr-4 bg-bg-secondary border border-border rounded-xl text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-accent/50 transition-all"
          />
        </div>
        <button type="submit" className="px-5 h-11 rounded-xl bg-accent hover:bg-accent-hover text-white text-sm font-semibold transition-all">
          Search
        </button>
        <button
          type="button"
      ...
      {/* Filters panel */}
      {showFilters && (
        <div className="mb-6 p-5 bg-bg-secondary border border-border rounded-xl animate-slide-down space-y-5">
          {/* Sort */}
          <div>
            <p className="text-xs font-medium text-text-muted uppercase tracking-wider mb-2">Sort By</p>
          ...
          {/* Status */}
          <div>
            <p className="text-xs font-medium text-text-muted uppercase tracking-wider mb-2">Status</p>
            <div className="flex flex-wrap gap-2">
          ...
          {/* Demographic */}
          <div>
            <p className="text-xs font-medium text-text-muted uppercase tracking-wider mb-2">Demographic</p>
            <div className="flex flex-wrap gap-2">
          ...
          {/* Content Rating */}
          <div>
            <p className="text-xs font-medium text-text-muted uppercase tracking-wider mb-2">Content Rating</p>
            <div className="flex flex-wrap gap-2">
          ...
          {genreTags.length > 0 && (
            <div>
              <p className="text-xs font-medium text-text-muted uppercase tracking-wider mb-2">Genres</p>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                        selected ? 'bg-accent-secondary text-white' : 'bg-bg-tertiary text-text-secondary hover:text-text-primary'
                      }`}
                    >
                      {name}
                    </button>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-accent hover:bg-accent-hover text-white text-sm font-semibold transition-all"
          >
            Apply Filters
          </button>
        </div>
          ...
          {offset < total && (
            <div className="text-center mt-8">
              <button
                ...
                disabled={loading}
                className="px-6 py-2.5 rounded-xl bg-bg-secondary border border-border text-sm font-medium text-text-secondary hover:text-text-primary hover:border-border-hover transition-all disabled:opacity-50"
              >
